    ...
    public static function testInstall(Smarty $smarty, &$errors = null)
    {
        ...
        if (is_dir($source)) {
            ...
            if ($expectedSysplugins) {
                $status = false;
                $message = "FAILED: files missing from lib/sysplugins: " . join(', ', array_keys($expectedSysplugins));
                if ($errors === null) {
                    echo $message . ".\n";
                } else {
        ...
        if (is_dir($source)) {
            ...
            if ($expectedPlugins) {
                $status = false;
                $message = "FAILED: files missing from lib/plugins: " . join(', ', array_keys($expectedPlugins));
                if ($errors === null) {
                    echo $message . ".\n";
                } else {
